...
function displayMaterials(grouped) {
  ...
  for (const subject in grouped) {
    ...
    grouped[subject].forEach((file) => {
      ...
      const starsRow = card.querySelector('.stars-row');
      if (starsRow && sdb) {
        starsRow.style.cursor = 'pointer';
        starsRow.addEventListener('click', async (e) => {
          ...
          if (!sdb.isReady) return;

          // Optimistic UI — immediately show selected stars
          starsRow.querySelectorAll('.star').forEach((s, i) => {
            s.className = i < chosenStar ? 'star filled' : 'star';
            s.innerHTML = i < chosenStar
              ? '<i class="fa-solid fa-star"></i>'
              : '<i class="fa-regular fa-star"></i>';
          });

          const result = await sdb.rateMaterial(file.folder, file.name, chosenStar);

          if (result && result.success) {
            // Update display with confirmed server values
            const newRating = result.rating || chosenStar;
            const newCount = result.count || 1;
            const fullStars = Math.floor(newRating);
            const hasHalf = (newRating - fullStars) >= 0.5;
            starsRow.querySelectorAll('.star').forEach((s, i) => {
              const n = i + 1;
              if (n <= fullStars) {
                ...
                s.innerHTML = '<i class="fa-solid fa-star-half-stroke"></i>';
              } else {
                s.className = 'star';
                s.innerHTML = '<i class="fa-regular fa-star"></i>';
              }
            });
            const ratingSmall = starsRow.closest('.meta-rating')?.querySelector('small');
            if (ratingSmall) ratingSmall.textContent = newRating.toFixed(1);
            starsRow.title = `${newRating.toFixed(1)} / 5 (${newCount} ratings)`;
          } else if (result && !result.success) {
            // Already rated — give subtle visual feedback
            starsRow.title = result.message || 'Already rated this session';
          }
        });
      }
      // ────────────────────────────────────────────────────────────────────
